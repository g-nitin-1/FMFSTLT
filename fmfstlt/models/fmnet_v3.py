...
class FMNetV3(nn.Module):
    ...
    def __init__(self, config: FMNetV3Config) -> None:
        ...
        self.position_embedding = nn.Parameter(torch.zeros(1, config.max_buckets, d))
    ...
    def encode(self, x_full: torch.Tensor) -> torch.Tensor:
        ...
        if F != self.config.feature_dim:
            raise ValueError(f"feature dim mismatch: expected {self.config.feature_dim}, got {F}")
        if T > self.config.max_buckets:
            raise ValueError(f"sequence length {T} exceeds max_buckets {self.config.max_buckets}")
