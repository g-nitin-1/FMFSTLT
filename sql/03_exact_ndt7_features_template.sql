-- Feature-table template for the exact ndt7 rebuild.
--
-- Run this file three times by changing SOURCE_META_TABLE and TARGET_TABLE.
--
-- Recommended SOURCE_META_TABLE values:
--   ee-21cs01007.fmfstlt.paper_exact_train_800k_meta
--   ee-21cs01007.fmfstlt.paper_exact_test_40k_meta
--   ee-21cs01007.fmfstlt.paper_exact_robustness_133k_meta
--
-- Recommended TARGET_TABLE values:
--   ee-21cs01007.fmfstlt.paper_exact_train_features_100ms_public
--   ee-21cs01007.fmfstlt.paper_exact_test_features_100ms_public
--   ee-21cs01007.fmfstlt.paper_exact_robustness_features_100ms_public
--
-- Important:
-- - this template is paper-faithful on source selection and throughput reconstruction
-- - `pipe_full_samples` is derived from public BBR BW samples using the standard
--   BBR startup logic: a substantial BW growth reset uses a 1.25x threshold and
--   pipe-full is considered reached after 3 consecutive non-growth signals
-- - the first 10 seconds are modeled as 100 buckets: elapsed_us < 10,000,000

DECLARE SOURCE_META_TABLE STRING DEFAULT 'ee-21cs01007.fmfstlt.paper_exact_train_800k_meta';
DECLARE TARGET_TABLE STRING DEFAULT 'ee-21cs01007.fmfstlt.paper_exact_train_features_100ms_public';
...
annotated AS (
  SELECT
    ...
    -- Official ndt7 download throughput is computed from BytesAcked / ElapsedTime.
    -- For the instantaneous feature, use the delta form when possible and fall back
    -- to the cumulative form for the first sample.
    COALESCE(
      8 * SAFE_DIVIDE(
    ...
    CAST(
      SAFE_MULTIPLY(CAST(unacked AS FLOAT64), CAST(snd_mss AS FLOAT64))
      AS FLOAT64
    ) AS bytes_in_flight,
...
--
-- SELECT
--   MIN(bucket_100ms) AS min_bucket,
--   MAX(bucket_100ms) AS max_bucket,
--   COUNT(*) AS rows
-- FROM `ee-21cs01007.fmfstlt.paper_exact_train_features_100ms_public`;
--
-- SELECT
--   APPROX_COUNT_DISTINCT(uuid) AS tests,
--   COUNT(*) AS rows
-- FROM `ee-21cs01007.fmfstlt.paper_exact_train_features_100ms_public`;
