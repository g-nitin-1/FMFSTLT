-- Canonical robustness feature-table build. Run after 01 and 02.
-- scripts/build_exact_public_bigquery.sh replaces the default project/dataset
-- identifiers for portable execution.

CREATE OR REPLACE TABLE `ee-21cs01007.fmfstlt.paper_exact_robustness_features_100ms_public` AS
WITH raw_measurements AS (
  ...
  FROM `ee-21cs01007.fmfstlt.paper_exact_robustness_133k_meta` AS m
  ...
  WHERE sm.TCPInfo IS NOT NULL
    AND t.date IN ('2025-02-06', '2025-03-04')
